...
public class SoundRayScatter
{
    ...
    public static Stack<SoundRay> angle360(Vector origin, int noOfRays, double maxDist, int bounces, SoundRay parent)
    {
        Stack<SoundRay> rays = new Stack<>();
        ...
        for(int i = 0; i < 360; i += increment)
        {
            rays.add(new SoundRay(origin, origin.add(dir.rotate(i)), bounces-1, parent));
        }
        return rays;
    }
}
